...
export const COLLECTIONS: CollectionDefinition[] = [
  ...
  },
  {
    ...
    plural: "Monitoring",
    description:
      "Endpoints checked for the public status page. Visitors never receive the probe URL — only the display name, logo, category, and an optional homepage link.",
    icon: "Activity",
    group: "Operations",
    titleField: "name",
    writeRole: "ADMIN",
    ...
    guidance:
      "HTTP checks run from Canadian vantage points, not from this website. Company-owned hosts are never sent out. The probe URL stays on the server and is never sent to visitors. A homepage link on the card is optional and separate. Rows on the internal denylist stay off the public page even if Public is checked.",
    listColumns: [
      { field: "name", label: "Name" },
      { field: "category", label: "Category" },
